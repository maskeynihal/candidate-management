...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.candidate-official-details.update", [$candidateOfficialDetail->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="official_email">{{ trans('cruds.candidateOfficialDetail.fields.official_email') }}</label>
                <input class="form-control {{ $errors->has('official_email') ? 'is-invalid' : '' }}" type="email" name="official_email" id="official_email" value="{{ old('official_email', $candidateOfficialDetail->official_email) }}" required>
