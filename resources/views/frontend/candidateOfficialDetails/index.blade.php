        ...
        <div class="col-md-12">
            @can('candidate_official_detail_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.candidate-official-details.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $candidateOfficialDetail->id }}">
                                        ...
                                        <td>
                                            @can('candidate_official_detail_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.candidate-official-details.show', $candidateOfficialDetail->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('candidate_official_detail_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.candidate-official-details.edit', $candidateOfficialDetail->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('candidate_official_detail_delete')
                                                <form action="{{ route('frontend.candidate-official-details.destroy', $candidateOfficialDetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('candidate_official_detail_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.candidate-official-details.massDestroy') }}",
