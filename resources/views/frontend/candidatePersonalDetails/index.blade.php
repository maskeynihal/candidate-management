        ...
        <div class="col-md-12">
            @can('candidate_personal_detail_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.candidate-personal-details.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.candidatePersonalDetail.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'CandidatePersonalDetail', 'route' => 'admin.candidate-personal-details.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-CandidatePersonalDetail">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $candidatePersonalDetail->id }}">
                                        ...
                                        <td>
                                            @can('candidate_personal_detail_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.candidate-personal-details.show', $candidatePersonalDetail->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('candidate_personal_detail_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.candidate-personal-details.edit', $candidatePersonalDetail->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('candidate_personal_detail_delete')
                                                <form action="{{ route('frontend.candidate-personal-details.destroy', $candidatePersonalDetail->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('candidate_personal_detail_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.candidate-personal-details.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-CandidatePersonalDetail:not(.ajaxTable)').DataTable({ buttons: dtButtons })
